...
namespace InitWindow {
    void InitWindowClass::callback_FramebufferSize(GLFWwindow* window, int width, int height) {
        window_size = glm::ivec2(width, height);
        callback_FramebufferSize_custom(width, height);
    }
    void InitWindowClass::callback_WindowPos(GLFWwindow* window, int xpos, int ypos) {
        window_position = glm::ivec2(xpos, ypos);
        callback_WindowPos_custom(xpos, ypos);
    }
    void InitWindowClass::callback_CursorPos(GLFWwindow* window, double xpos, double ypos) {
    ...
    void framebuffer_size_callback(GLFWwindow* window, int width, int height) {
        auto* self = static_cast<InitWindowClass*>(glfwGetWindowUserPointer(window));
        if(self) {
